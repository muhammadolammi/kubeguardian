...
from tools import test # add tools to add here
...
#TODO create map of tools instead

test_tool = FunctionTool(test)
tools  = {
    "test_tool": test_tool
}
names = []
...
print(f"ADK tools '{names}' initialized and ready to be exposed via MCP.")
# --- End ADK Tool Prep ---
...
async def list_mcp_tools() -> list[mcp_types.Tool]:
    ...
    mcp_schemas_for_function_tools = []
    for _, function_tool in tools:
        mcp_schemas_for_function_tools.append(adk_to_mcp_tool_type(function_tool))
    # mcp_tool_schema = adk_to_mcp_tool_type(adk_tool_to_expose)
    print(f"MCP Server: Advertising tools: {[mcp_tool_schema.name for mcp_tool_schema in mcp_schemas_for_function_tools]  }")
    return mcp_schemas_for_function_tools
...
async def call_mcp_tool(
    ...
    # TODO check if its in list of available tool instead
    ...
    if tools[name]:
        try:
            ...
            adk_tool_response = await tools[name].run_async(
                args=arguments,
                tool_context=None,
            )
